...
func NewDev(v *validator.Validator, e *config.Env) *App {
	...
	createAccounts := usecase.NewCreateAccounts(v, mockpluggyClient, pgxTX, userRepo, accountRepo, accountBalanceRepo, institutionRepo, syncTransactions)
	...
	routerRouter := router.NewRouter(e, middlewareMiddleware, docHandler, authHandler, calculatorHandler, institutionHandler, categoryHandler, budgetHandler, userHandler, accountHandler, transactionHandler, balanceHandler, feedbackHandler, paymentMethodHandler, aiChatHandler, aiChatMessageHandler)
...
func NewProd(v *validator.Validator, e *config.Env) *App {
	...
	createAccounts := usecase.NewCreateAccounts(v, client, pgxTX, userRepo, accountRepo, accountBalanceRepo, institutionRepo, syncTransactions)
	accountHandler := handler.NewAccountHandler(createAccounts)
	getTransaction := usecase.NewGetTransaction(transactionRepo)
	updateTransaction := usecase.NewUpdateTransaction(v, transactionRepo)
	createTransaction := usecase.NewCreateTransaction(v, transactionRepo, userRepo, transactionCategoryRepo, paymentMethodRepo)
