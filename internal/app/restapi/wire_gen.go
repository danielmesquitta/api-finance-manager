...
func NewDev(v *validator.Validator, e *env.Env) *App {
	...
	signIn := usecase.NewSignIn(v, pgxTX, hasher, userRepo, userAuthProviderRepo, jwt, googleOAuth, mockOAuth)
	...
	createAccounts := usecase.NewCreateAccounts(v, mockpluggyClient, pgxTX, accountRepo, accountBalanceRepo, institutionRepo, userInstitutionRepo)
	...
	getTransactionByID := usecase.NewGetTransaction(transactionRepo)
	...
	transactionHandler := handler.NewTransactionHandler(syncTransactions, listTransactions, getTransactionByID, updateTransaction, createTransaction)
	getBalance := usecase.NewGetBalance(v, transactionRepo, accountBalanceRepo)
	syncBalances := usecase.NewSyncBalances(e, pgxTX, mockpluggyClient, redisCache, accountRepo, accountBalanceRepo)
	...
	createAIChat := usecase.NewCreateAIChat(v, aiChatRepo)
	...
	listAIChatMessagesAndAnswers := usecase.NewListAIChatMessagesAndAnswers(aiChatRepo)
	...
	generateAIChatMessage := usecase.NewGenerateAIChatMessage(v, pgxTX, dbDB, openAI, aiChatRepo, aiChatMessageRepo, aiChatAnswerRepo, paymentMethodRepo, transactionCategoryRepo, institutionRepo)
	aiChatHandler := handler.NewAIChatHandler(createAIChat, deleteAIChat, updateAIChat, listAIChats, listAIChatMessagesAndAnswers, generateAIChatMessage)
	routerRouter := router.NewRouter(e, middlewareMiddleware, healthHandler, docHandler, authHandler, calculatorHandler, institutionHandler, categoryHandler, budgetHandler, userHandler, accountHandler, transactionHandler, balanceHandler, feedbackHandler, paymentMethodHandler, aiChatHandler)
...
func NewProd(v *validator.Validator, e *env.Env) *App {
	...
	accountBalanceRepo := pgrepo.NewAccountBalanceRepo(dbDB)
	userInstitutionRepo := pgrepo.NewUserInstitutionRepo(dbDB)
	createAccounts := usecase.NewCreateAccounts(v, client, pgxTX, accountRepo, accountBalanceRepo, institutionRepo, userInstitutionRepo)
	accountHandler := handler.NewAccountHandler(createAccounts)
	redisCache := rediscache.NewRedisCache(e)
	paymentMethodRepo := pgrepo.NewPaymentMethodRepo(dbDB)
	...
	transactionHandler := handler.NewTransactionHandler(syncTransactions, listTransactions, getTransactionByID, updateTransaction, createTransaction)
	getBalance := usecase.NewGetBalance(v, transactionRepo, accountBalanceRepo)
	syncBalances := usecase.NewSyncBalances(e, pgxTX, client, redisCache, accountRepo, accountBalanceRepo)
	balanceHandler := handler.NewBalanceHandler(getBalance, syncBalances)
	feedbackRepo := pgrepo.NewFeedbackRepo(dbDB)
	createFeedback := usecase.NewCreateFeedback(v, feedbackRepo)
	feedbackHandler := handler.NewFeedbackHandler(createFeedback)
